...
node {
   ...
   stage('Start deploy') {
     withCredentials([usernamePassword(credentialsId: 'ghe-token', passwordVariable: 'PASSWORD', usernameVariable: 'USERNAME')]) {
         ...
         sleep 3
         curl -d '{"state": "success", "environment": "qa", "description": "All done", "target_url": "http://www.google.com"}' -X POST -H "Authorization: token $PASSWORD" "https://api.github.com/repos/tesuvant/deployments_api_test/deployments/\$ID/statuses"
